...
{var map=_map;var projection=new Fluster2ProjectionOverlay(map,this);var me=this;var clusters=new Object();var markersLeft=new Object();this.debugEnabled=_debug;this.gridSize=60;this.minimumClusterSize=1;this.maximumZoomLevel=19;this.markers=new Array();this.currentZoomLevel=-1;this.styles={0:{image:'http://gmaps-utility-library.googlecode.com/svn/trunk/markerclusterer/1.0/images/m1.png',textColor:'#FFFFFF',width:53,height:52},10:{image:'http://gmaps-utility-library.googlecode.com/svn/trunk/markerclusterer/1.0/images/m2.png',textColor:'#FFFFFF',width:56,height:55},20:{image:'http://gmaps-utility-library.googlecode.com/svn/trunk/markerclusterer/1.0/images/m3.png',textColor:'#FFFFFF',width:66,height:65}};var zoomChangedTimeout=null;this.createClusters=function()
...
{me.debug("Nothing to do here since changes of 1.2.0");};this.clear=function()
{for(var i=0;i<me.markers.length;i++)
{me.markers[i].setMap(null);}
me.markers=[];for(var i=0;i<20;i++)
{if(!clusters[i])
{continue;}
...
{if(this.markers.length==1||this.markers.length<this.fluster.minimumClusterSize||this.fluster.currentZoomLevel>this.fluster.maximumZoomLevel)
...
{this.fluster=_fluster;this.cluster=_cluster;this.position=this.cluster.getPosition();this.markerCount=this.cluster.getMarkerCount();this.map=this.fluster.getMap();this.style=null;this.div=null;var styles=this.fluster.getStyles();for(var i in styles)
{if(this.markerCount>i)
{this.style=styles[i];}
else
{break;}}
google.maps.OverlayView.call(this);this.setMap(this.map);this.draw();};Fluster2ClusterMarker.prototype=new google.maps.OverlayView();Fluster2ClusterMarker.prototype.draw=function()
{if(this.div==null)
{var me=this;this.div=document.createElement('div');this.div.style.position='absolute';this.div.style.width=this.style.width+'px';this.div.style.height=this.style.height+'px';this.div.style.lineHeight=this.style.height+'px';this.div.style.background='transparent url("'+this.style.image+'") 50% 50% no-repeat';this.div.style.color=this.style.textColor;this.div.style.textAlign='center';this.div.style.fontFamily='Arial, Helvetica';this.div.style.fontSize='11px';this.div.style.fontWeight='bold';this.div.innerHTML=this.markerCount;this.div.style.cursor='pointer';google.maps.event.addDomListener(this.div,'click',function(){me.map.fitBounds(me.cluster.getMarkerBounds());});this.getPanes().overlayLayer.appendChild(this.div);}
var position=this.getProjection().fromLatLngToDivPixel(this.position);this.div.style.left=(position.x-parseInt(this.style.width/2))+'px';this.div.style.top=(position.y-parseInt(this.style.height/2))+'px';};Fluster2ClusterMarker.prototype.hide=function()
{this.div.style.display='none';};Fluster2ClusterMarker.prototype.show=function()
{this.div.style.display='block';};function Fluster2ProjectionOverlay(map,fluster)
{google.maps.OverlayView.call(this);this.setMap(map);this.fluster=fluster;this.previousZoomLevel=null;this.getP=function()
...
{this.fluster.createClusters();this.previousZoomLevel=this.map.getZoom();}
