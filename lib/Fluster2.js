...
function Fluster2(_map, _debug)
{
	// Private variables
	var map = _map;
	var projection = new Fluster2ProjectionOverlay(map, this);
	var me = this;
	 ...
	 */
	this.showClustersInBounds = function()
	{
	...
	{		
		me.debug("Nothing to do here since changes of 1.2.0");
	};
	
	/**
	 * Removes all markers from clusters and destroys them.
	 */
	this.clear = function()
	{
		// Unset markers
		...
		for(var i = 0; i < 20; i++)
		{
			// Skip if no clusters at this zoom level
			if(!clusters[i])
			{
				continue;
			}
