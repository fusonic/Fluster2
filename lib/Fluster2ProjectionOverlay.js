 ...
 */
function Fluster2ProjectionOverlay(map, fluster)
{
...
{
	// Recalculate clusters if zoom level changed
	if(this.map.getZoom() != this.previousZoomLevel)
	{
		this.fluster.createClusters();
		this.previousZoomLevel = this.map.getZoom();
	}

	// Show clusters in bounds
	this.fluster.showClustersInBounds();
};